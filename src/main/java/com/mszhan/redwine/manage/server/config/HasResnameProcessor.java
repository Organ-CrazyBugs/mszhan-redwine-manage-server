...
public class HasResnameProcessor extends AbstractAttributeModifierAttrProcessor {
	private static final String ROLE_CLASS = "security_role_class";

 

	public HasResnameProcessor() {
		super("secResName");
	}
...
//		Map<String, Object> map = (Map<String, Object>) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
		...
		try {
			moduleCodeList = (List<String>) map.get("moduleCodeList");
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		for (String attr : attrList) {
			if (moduleCodeList.contains(attr)) {
				values.clear();
				return values;
			}
		}
